...
- **Version-controlled rules:** Keep all rules in `.cursor/rules/` (and optionally `.agents/skills/`) under git so every team member and every AI session uses the same standards. Changes go through review like code.
...
- **Agent skills:** Use `.agents/skills/` for deeper, checklist-style guidance (e.g. “Before finalizing: no workarounds? Risks communicated?”). Skills are referenced when doing feature work or reviews.
...
| **`.agents/skills/`** | Deeper checklists and patterns (e.g. anti-patterns, solution-quality skill). Used for feature work and code review. |
...
- **`.agents/skills/ai-solution-quality/SKILL.md`** — Checklist for the AI (and developers) when implementing features: avoid workarounds, prefer proper design, communicate risks, ask before proceeding on suboptimal choices. Referenced in `project.md` and `.agents/skills/README.md`.

## Maintenance

- **Rule owners:** Designate an owner (e.g. tech lead) for `.cursor/rules/` and `.agents/skills/`. They update rules when patterns or frameworks change and resolve conflicts.
...
- Skills alignment: `.agents/skills/README.md`.
