...
- Follow PEP 8. Use **black** (line length 88) and **isort** for formatting. Config: `.flake8`, `backend/Makefile`, `.pre-commit-config.yaml`.
- Do not copy full style guides into rules; run `make format` and `make lint`. For detailed style, see `PEP8_STYLE_GUIDE.md` in the repo.
...
- Do not remove or bypass pre-commit hooks or make targets for format/lint.
