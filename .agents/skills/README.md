...
| **python-design-patterns** | project structure (endpoints → services → repositories), Depends() for DI in FastAPI, clean-code (SRP, separation of concerns). |
...
| **python-performance-optimization** | Async I/O in API path; blocking only via `asyncio.to_thread`. DB: use AsyncSession, pooling, batching; see database-postgres.md. After changes: run `make format` and `make lint`. |
